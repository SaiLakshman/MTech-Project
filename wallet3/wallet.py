...
class Wallet(object):
    _users = None
    _USER_FILENAME= '/home/sailakshman/Desktop/MedRec/wallet3/users.txt'   
    @property
    def users(self):
        if not self._users:
...
#         self.__store_users()
    ...
    def get_ec_public_key(self,user= None):
        user= self.get_username_given_public_key(user)
        if user not in self.users:
           raise UnregisteredUserError(username + ' is not a registered user')
        details= self.users[user]
        return details['ec_pubkey']
...
if __name__ =='__main__':
   ...
   w.add_user(u)
   w.add_user(u1)
   #u.from_dict(w.users["hello"])
   u.generate_shared_secret(u1)
